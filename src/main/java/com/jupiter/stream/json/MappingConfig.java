...
public class MappingConfig {
    ...
    private MappingData getMappingDataForColumn(String keyField) {
        for (MappingData mdata: mapping) {
            if(keyField.equalsIgnoreCase(mdata.getValue())) {
                log.info("Mapping is found for column: " + keyField);
                return mdata;
            }
        }

        log.info("Mapping is NOT found for column: " + keyField);
        return null;
    }
